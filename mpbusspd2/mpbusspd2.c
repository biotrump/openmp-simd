/*
  gcc mpbusspd2.c cpuidc.c -lpthread -lm -lrt -O3 -mcpu=cortex-a7 -mfloat-abi=hard -mfpu=neon-vfpv4 -o MP-BusSpd2PiA7
  cc  mpbusspd2.c cpuidc.c -lrt -lc -lm -O3 -o MP-BusSpd2

  taskset 0x00000001 ./MP-BusSpd2PiA7
 */
...
void main()
{
    ...
    printf("   Staggered starting addresses to avoid caching\n\n");
    printf("  KB      Inc32  Inc16   Inc8   Inc4   Inc2  RdAll\n\n");

    fprintf(outfile, "\n MP-BusSpd %s %s\n", version, timeday);
    fprintf(outfile, "   MB/Second Reading Data, 1, 2, 4 and 8 Threads\n");
    fprintf(outfile, "   Staggered starting addresses to avoid caching\n\n");
...
{
   int i, j, k;
   int length = wordsToTest / threads;
   int start  = t * length;
